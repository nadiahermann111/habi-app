...
if __name__ == "__main__":
    ...
    @app.post("/api/coins/add")
    async def add_coins(coins_data: dict, authorization: str = Header(None)):
        """
        Dodaje monety do konta użytkownika.

        Args:
            coins_data (dict): Słownik zawierający pole 'coins' z liczbą monet do dodania
            authorization (str): Token autoryzacyjny w headerze

        Returns:
            dict: Informacja o dodanych monetach, nowa liczba monet

        Raises:
            HTTPException: Gdy token jest nieprawidłowy lub kwota <= 0
        """
        if not authorization or not authorization.startswith("Bearer "):
            raise HTTPException(status_code=401, detail="Brak tokenu autoryzacji")
        ...
        coins_to_add = coins_data.get("coins", 0)

        if coins_to_add <= 0:
            raise HTTPException(status_code=400, detail="Liczba monet musi być większa niż 0")

        print(f"💰 Adding {coins_to_add} coins to user {user_id}")
        ...
        async with aiosqlite.connect("database.db") as db:
            ...
            print(f"   ✅ User {user_id} now has {new_coins} coins (was {current_coins})")

            return {
                "message": f"Dodano {coins_to_add} monet",
                "coins": new_coins,
                "added": coins_to_add
            }
    ...
    @app.get("/api/slot-machine/can-play")
    async def can_play_slot_machine(authorization: str = Header(None)):
        """
        Sprawdza czy użytkownik może dziś zagrać w automat.

        Args:
            authorization (str): Token autoryzacyjny w headerze

        Returns:
            dict: can_play (bool), last_play_date (str|None), time_until_reset (str|None)
        """
        if not authorization or not authorization.startswith("Bearer "):
            raise HTTPException(status_code=401, detail="Brak tokenu autoryzacji")
        ...
        async with aiosqlite.connect("database.db") as db:
            ...
            cursor = await db.execute(
                "SELECT last_slot_play FROM users WHERE id = ?",
                (user_id,)
            )
            user = await cursor.fetchone()
            ...
            last_play_date = user["last_slot_play"]
            today = date.today().isoformat()

            # ✅ POPRAWKA: NULL oznacza że użytkownik nigdy nie grał
            if last_play_date is None:
                ...
                can_play = False
                # Oblicz czas do resetu
                from datetime import datetime as dt
                now = dt.now()
                tomorrow = dt(now.year, now.month, now.day) + timedelta(days=1)
                diff = tomorrow - now
                hours = diff.seconds // 3600
                minutes = (diff.seconds % 3600) // 60
                time_until_reset = f"{hours}h {minutes}m"

            print(f"🎰 Slot machine status for user {user_id}:")
            print(f"   last_play_date: {last_play_date}")
            print(f"   today: {today}")
            print(f"   can_play: {can_play}")
    ...
    async def play_slot_machine(authorization: str = Header(None)):
        """
        Zapisuje datę gry w automacie dla użytkownika.

        Args:
            authorization (str): Token autoryzacyjny w headerze

        Returns:
            dict: Potwierdzenie zapisania

        Raises:
            HTTPException: Gdy użytkownik już dzisiaj grał
        """
        if not authorization or not authorization.startswith("Bearer "):
            raise HTTPException(status_code=401, detail="Brak tokenu autoryzacji")
        ...
        print(f"🎰 User {user_id} próbuje zagrać w automat ({today})")
        ...
        async with aiosqlite.connect("database.db") as db:
            ...
            print(f"   📅 Last play date: {last_play_date}")
            print(f"   📅 Today: {today}")

            # ✅ POPRAWKA: Sprawdź czy już dzisiaj grał (NULL jest OK)
            if last_play_date == today:
                print(f"   ⚠️ User {user_id} already played today!")
                raise HTTPException(status_code=400, detail="Już dzisiaj zagrałeś w automat")

            # Zapisz datę gry
            await db.execute(
                "UPDATE users SET last_slot_play = ? WHERE id = ?",
                (today, user_id)
            )
            await db.commit()

            # Sprawdź czy faktycznie zapisało się
            cursor = await db.execute(
                "SELECT last_slot_play FROM users WHERE id = ?",
                (user_id,)
            )
            updated_user = await cursor.fetchone()
            print(f"   ✅ Updated last_slot_play to: {updated_user['last_slot_play']}")
    ...
    @app.post("/api/slot-machine/reset")
    async def reset_slot_machine(authorization: str = Header(None)):
        """
        TYLKO DEV: Resetuje automat dla użytkownika.
        """
        if not authorization or not authorization.startswith("Bearer "):
            raise HTTPException(status_code=401, detail="Brak tokenu autoryzacji")
        ...
        async with aiosqlite.connect("database.db") as db:
            ...
            # Wyzeruj last_slot_play
            await db.execute(
                "UPDATE users SET last_slot_play = NULL WHERE id = ?",
                (user_id,)
            )
            await db.commit()

            print(f"🔧 DEV: Reset slot machine for user {user_id}")

            return {
                "message": "Automat zresetowany",
                "user_id": user_id
            }
